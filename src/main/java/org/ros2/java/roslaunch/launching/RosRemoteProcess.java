...
import org.ros2.java.roslaunch.logging.FileLog;
import org.ros2.java.roslaunch.logging.PrintLog;
import org.ros2.java.roslaunch.logging.FileLog.FileLogger;
import org.ros2.java.roslaunch.parsing.MachineTag;
import org.ros2.java.roslaunch.util.EnvVar;
import org.ros2.java.roslaunch.util.Util;
 ...
 *
 * This class is responsible for implementing the RosProcessIF to start and
 * monitor a process that runs on a remote machine.
 */
public class RosRemoteProcess implements RosProcessIF {
	 ...
	 */
	public RosRemoteProcess(final String name, final String runId, final String masterUri, final MachineTag machine) {
		m_name = name;
		...
		if (envLoader == null) {
			throw new RuntimeException("machine.env_loader must have been assigned before creating ssh child instance");
		}

		// Create the set of arguments passed used to launch the child machine
		String[] args = new String[] { envLoader, "roslaunch", "-c", name, "-u", masterUri, "--run_id", runId, };
	...
	public boolean start() {
		...
		try {
			...
			PrintLog.info("remote[" + m_name + "]: creating ssh connection to " + address + ":" + sshPort + username);
			m_logger.info("remote[" + m_name + "]: invoking with ssh exec args [" + m_command + "]");

			this.execute(m_command);
		} catch (Exception e) {
			PrintLog.error("remote[" + m_name + "]: failed to launch on " + machine.getName() + "\n\n" + e.getMessage()
					+ "\n\n");
	 ...
	 * @throws RuntimeException if connections to unknown hosts are not allowed, and
	 *                          the machine is not in the known_hosts file
	 */
	private void execute(final String inCommand) {
		...
		if (m_session != null) {
			PrintLog.info("launching remote roslaunch child with command: [" + command + "]");
	...
	private void connect(final JSch jsch) {
		...
		try {
			...
			throw new RuntimeException("network error connecting to [" + address + ":" + port + "]: " + e.getMessage());
		}
	}
	...
	private void executeCommand(final String command) {
		...
		try {
			...
			m_logger.error(ExceptionUtils.getStackTrace(e));
			throw new RuntimeException("Failed to execute remote command: [" + command + "]\n" + e.getMessage());
		}
	}
	...
	/**
	 * Get the exit code for this process, or null if the process is still running.
	 *
	...
	public void restart() throws Exception {
		throw new RuntimeException("Remote processes do not support respawning at this time");
	}
	...
	public String getExitCodeDescription() {
		if (m_exitCode != -1) {
			String output = "";
			if (m_exitCode != 0) {
				output = "process has died [exit code: " + m_exitCode + ", cmd: " + m_command + "]";
			} else {
	...
	private boolean findKnownHost(final String address, final JSch jsch) {
		...
		try {
			...
			m_logger.error(ExceptionUtils.getStackTrace(e));
			throw new RuntimeException("cannot load SSH host keys -- your known_hosts file may be corrupt");
		}
	...
	/**
	 * The AllowUnknownHostsUserInfo class implements the UserInfo interface to
	 * allow connections to unknown hosts to be made. This class is used to allow
	 * SSH connections to unknown hosts automatically accept the yes/no unknown host
	 * prompt.
	 */
