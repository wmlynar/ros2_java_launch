...
import org.ros2.java.roslaunch.ArgumentParser;
import org.ros2.java.roslaunch.logging.PrintLog;
import org.ros2.java.roslaunch.parsing.GroupTag;
import org.ros2.java.roslaunch.parsing.IncludeTag;
import org.ros2.java.roslaunch.parsing.LaunchFile;
import org.ros2.java.roslaunch.parsing.NodeTag;
import org.ros2.java.roslaunch.parsing.ParamTag;
...
public class NodeManager
{
	 ...
	 */
	public static CommandInDirectory getNodeCommandLine(
			final NodeTag node,
			final boolean addNamespace)
	{
		CommandInDirectory ret = new CommandInDirectory();
		...
		ret.directory = executable.getParentFile().getAbsolutePath(); 
		...
		List<ParamTag> params = node.getParams();
		for (ParamTag param : params)
		{
			fullCommand.add("_" + param.getParamName() + ":=" + param.getValue());
		}
			...
			final String masterUri) throws IOException
	{
		...
		CommandInDirectory commandInDirectory = getNodeCommandLine(node, false);
		String[] command = commandInDirectory.commandLine;
		...
		File workingDir = new File(commandInDirectory.directory);
...
//		}
		...
		Process proc = launchNodeProcess(
				processName, uuid, command, envp, workingDir, false);
