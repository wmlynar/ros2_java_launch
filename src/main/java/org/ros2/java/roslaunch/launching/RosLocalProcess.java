...
import org.ros2.java.roslaunch.util.RosUtil;
import org.ros2.java.roslaunch.util.Util;

/**
 * The RosProcess class
 *
 * This class encapsulates a named running process. This class implements the
 * RosProcessIF to monitor a process running on a local machine.
 */
public class RosLocalProcess implements RosProcessIF {
	 ...
	 * @param logStreams true if the stdout and stderr streams are being logged to a
	 *                   file
	 */
	public RosLocalProcess(final String name, final Process process, final String[] command, final String uuid,
			final boolean isCore, final boolean required, final boolean logStreams) {
		m_name = name;
		m_process = process;
		m_uuid = uuid;
		m_isCore = isCore;
	...
	public String getExitCodeDescription() {
		...
		if (exitCode != null) {
			...
			if (exitCode != 0) {
				int pid = this.getPid();
				output = "process has died [pid: " + pid + ", exit code: " + exitCode + ", cmd: " + m_command + "]";
			} else {
	...
	public void restart() throws Exception {
		...
		// Restart the process
		m_process = NodeManager.launchNodeProcess(m_name, m_uuid, m_commandArgs, m_envp, m_workingDir, m_logStreams);
	...
	/**
	 * Replace the old __log command line argument to the node with the path to the
	 * latest log file. This is supposed to be used when respawning the node as the
	 * name of the process changes with each respawn.
	 */
	private void replaceOldLogArgument() {
		...
		for (String arg : m_command.split(" ")) {
			if (index++ > 0)
				command += " "; // Separate each arg with a space
